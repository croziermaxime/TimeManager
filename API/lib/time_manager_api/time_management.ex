defmodule TimeManagerApi.TimeManagement do
  @moduledoc """
  ...
  def get_last_clock_by_userid!(userId), do:
    from(w in Clock, where: w.user == ^userId, order_by: [desc: w.inserted_at], limit: 1)
    |> Repo.all()
  ...
  def get_workingtimes_of_user!(userId), do:
    from(w in Workingtime, where: w.user == ^userId)
    |> Repo.all()

  def get_workingtime_of_user_by_id!(id, userId), do:
    from(w in Workingtime, where: w.id == ^id and w.user == ^userId)
    |> Repo.all()


  def get_workingtime_of_user_by_time_range!(userId, start, endi), do:
    from(w in Workingtime,
         where: w.user == ^userId and w.start >= ^start and w.end <= ^endi)
    |> Repo.all()
  @doc """
